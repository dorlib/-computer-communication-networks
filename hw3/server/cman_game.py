...
class Game():
	def __init__(self, map_path):
		...
		self.points = {(i,j):  0 for i in range(self.board_dims[0]) 
                 for j in range(self.board_dims[1]) 
                 if self.board[i][j] == gm.POINT_CHAR}
		self.restart_game()

	def get_points_byte_string(self):
		"""
		Converts self.points_list into a byte string where each bit represents a point.
		
		Returns:
			bytes: A byte string where each bit represents a point in the board.
		"""
  
		points_list =  sorted(self.points.keys(), key=lambda x: (x[0], x[1]))

		    # Create the bit string
		bit_string = ''.join(str(self.points[point]) for point in points_list)

  		# Convert the bit string into bytes
		byte_array = int(bit_string, 2)
		bytes_list = [(byte_array >> (8 * i)) & 0xFF for i in range(4, -1, -1)]
		return bytes_list
	...
	def apply_move(self, player, direction):
		...
		p_coords = self.cur_coords[player-1]
		...
		if self.board[next_coords[0]][next_coords[1]] not in gm.PASS_CHARS:
			...
			self.cur_coords[player-1] = next_coords
